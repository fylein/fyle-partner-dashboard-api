...
export DATABASE_URL=postgres://postgres:postgres@db:5432/test_partner_dashboard_db
...
do
    ...
    PGPASSWORD=postgres psql -h $DB_HOST -U postgres test_partner_dashboard_db -c "\i $path"
done

# creating a dump of the new fixture
PGPASSWORD=postgres pg_dump -U postgres -h db -d test_partner_dashboard_db > tests/sql_fixtures/reset_db_fixtures/reset_db.sql
